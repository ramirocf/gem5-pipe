...
//  friend class Fi_System;
  //Pipeline Registers
  friend class Fetch_DecodeFaultQueue;
  friend class Decode_RenameFaultQueue;
  friend class Rename_IssueFaultQueue;
  friend class Issue_ExecuteFaultQueue;
  friend class WB_CommitFaultQueue;
  friend class Execute_WBFaultQueue;
...
public:
  ...
  //Pipeline Registers
  static const InjectedFaultType Fetch_DecodeFault = 9;
  static const InjectedFaultType Decode_RenameFault = 10;
  static const InjectedFaultType Rename_IssueFault = 11;
  static const InjectedFaultType Issue_ExecuteFault = 12;
  static const InjectedFaultType WB_CommitFault = 13;
  static const InjectedFaultType Execute_WBFault = 14;
  //End of Pipeline Registers
  InjectedFault *nxt;
  InjectedFault *prv;
...
public:
  ...
  bool manifest_bool(bool in, uint64_t out, InjectedFaultValueType type)
  {
    bool retVal;
    bool out_bool = (out) ? true : false;
    if (DTRACE(FaultInjection)) {
      std::cout << "HEX:Value before FI: " << type_to_hex(in) << "\n";
    }
    switch (type)
