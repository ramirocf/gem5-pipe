...
//Include for Pipeline Registers
...
#include "fi/execute_wb_fault.hh"
...
//Pipeline Reisters
class Fetch_DecodeFaultQueue;
class Decode_RenameFaultQueue;
class Rename_IssueFaultQueue;
class Issue_ExecuteFaultQueue;
class WB_CommitFaultQueue;
class Execute_WBFaultQueue;
//Pipeline Registers
...
class Fi_System : public MemObject
{
  ...
  public :
    ...
    //Pipeline Registers
    Fetch_DecodeFaultQueue fetch_DecodeFaultQueue; //("Fetch Decode Fault Queue")
    Decode_RenameFaultQueue decode_RenameFaultQueue;//("Decode Rename Fault Queue")
    Rename_IssueFaultQueue rename_IssueFaultQueue;//("Rename Issue Fault Queue")
    Issue_ExecuteFaultQueue issue_ExecuteFaultQueue;//("Issue Execute Fault Queue")
    WB_CommitFaultQueue wb_CommitFaultQueue;//("WriteBack-Commit Fault Queue")
    Execute_WBFaultQueue execute_WBFaultQueue;//("Execute-WriteBack Fault Queue")
    //End of Pipeline Registers
    /*
    ...
    bool fi_main;
    //Pipeline registers
    bool fi_fetch_decode;
    bool fi_decode_rename;
    bool fi_rename_issue;
    bool fi_issue_execute;
    bool fi_wb_commit;
    bool fi_execute_wb;
    //End of pipeline registers
  ...
  public: 
   ...
   //Fetch-Decode register fault
   template <class T>
   inline T fetch_decode_fault(ThreadContext *tc,ThreadEnabledFault *thread, T curInst,Addr pcAddr,int curBundle, int fieldType){
      Fetch_DecodeFault *fetch_DecodeFault = NULL;
      std::string _name = tc->getCpuPtr()->name();
      if(thread->getMode() != START || !FullSystem || !TheISA::inUserMode(tc))
        return curInst;
   ...
   //Decode-Rename register fault
   template <class T>
   inline T decode_rename_fault(ThreadContext *tc,ThreadEnabledFault *thread, T curInst,Addr pcAddr,int curBundle, int fieldType){
      Decode_RenameFault *decode_RenameFault = NULL;
      std::string _name = tc->getCpuPtr()->name();
      if(thread->getMode() != START || !FullSystem || !TheISA::inUserMode(tc))
        return curInst;
  ...
  //Rename-Issue Fault
   template <class T>
   inline T rename_issue_fault(ThreadContext *tc,ThreadEnabledFault *thread, T curInst,Addr pcAddr,int curBundle, int fieldType){
      Rename_IssueFault *rename_IssueFault = NULL;
      std::string _name = tc->getCpuPtr()->name();
      if(thread->getMode() != START || !FullSystem || !TheISA::inUserMode(tc))
        return curInst;
    ...
    //Issue Execute Fault
   template <class T>
   inline T issue_execute_fault(ThreadContext *tc,ThreadEnabledFault *thread, T curInst,Addr pcAddr,int curBundle, int fieldType){
      Issue_ExecuteFault *issue_ExecuteFault = NULL;
      std::string _name = tc->getCpuPtr()->name();
      if(thread->getMode() != START || !FullSystem || !TheISA::inUserMode(tc))
        return curInst;
    ...
    //WB Commit Fault
   template <class T>
   inline T wb_commit_fault(ThreadContext *tc,ThreadEnabledFault *thread, T curInst,Addr pcAddr,int curBundle, int fieldType){
      WB_CommitFault *wb_CommitFault = NULL;
      std::string _name = tc->getCpuPtr()->name();
      if(thread->getMode() != START || !FullSystem || !TheISA::inUserMode(tc))
        return curInst;
    ...
    //Execute WB Register Fault
    template <class MYVAL, class U>
      inline MYVAL execute_wb_fault(U ptr,ThreadContext *tc,MYVAL value, int curBundle, int fieldType){

        if(!( tc->getEnabledFI()) )
          return value;
        ThreadEnabledFault *thread = tc->getEnabledFIThread();
        Execute_WBFault *execute_WBFault = NULL;
        if( thread->getMode() == START && FullSystem && (TheISA::inUserMode(tc))   ){
          Addr pcAddr = ptr->instAddr();
          std::string _name = tc->getCpuPtr()->name();
  ...
  //SPEC_FI
  bool enable_trace;
  std::ofstream golden_trace,fi_trace;
  int increaseTicks(std :: string curCpu , ThreadEnabledFault *curThread , uint64_t ticks,ThreadContext *tc);
  void SPECFI_timing(ThreadContext *tc,ThreadEnabledFault *curThread,std::string curCpu);
  int check_arch();
  //SPEC_FI
